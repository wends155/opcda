...
- 异步写入标签的值
- 订阅标签的实时数据变化
- **鲁棒性**: 防御性 nil 检查和错误处理，防止运行时 panic。
...
全部 API 见 [API 文档](https://pkg.go.dev/github.com/wends155/opcda)
...
2. 多平台编译
   ...
   Windows 平台的接口设置为空实现。
    ...
    本客户端使用了 COM 接口，内部已经处理了内存释放，并进行了所有支持类型的疲劳测试。所有公共方法都增加了防御性 nil 检查，在对象未初始化时会优雅地返回错误而非触发 panic。
